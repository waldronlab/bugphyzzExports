...
for (i in seq_along(data_ready)) {
    message('Preparaing ', names(phys)[i], ' - ', Sys.time())
...
vct_lgl <- map_lgl(data_ready, is.null)
if (any(vct_lgl)) {
    message('Errors in ', sum(vct_lfl), 'data.frames.')
    data_ready <- discard(data_ready, is.null)
}
