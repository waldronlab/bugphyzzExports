...
.writeHeader <- function(file_path, header = .getHeader()) {
...
#' @param header a character vector representing the GMT header
#' 
...
.writeFileWithHeader <- function(signatures, file_path, header = .getHeader()) {
...
.makeFullDump <- function(file_path, ps = physiologies()) {
        ...
        for (tax.id.type in .TAX.ID.TYPES) {
            ...
            signatures <- append(signatures,
                                 .makeSignaturesByTaxIdAndLevel(ps,
                                                                tax.id.type,
                                                                tax.level))
            .writeFileWithHeader(signatures, file_path)
        }
    }
    .makeFullDump("full_dump.csv", all_ps)
}